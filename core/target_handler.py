...
class TargetHandler:
    ...
    def __init__(self, target_soup, path):
        self.target_soup = target_soup
        self.path = path

        self.metadata = self._get_metadata()
        self.target_id = self.metadata.get('target_id')
        self.is_bundle = self.metadata.get('is_bundle')
        self.token = self.metadata.get('token')
        self.title = self._get_title()
        self.platform = self._get_platform()
        self.discount = self._get_discount()
        self.released_date = self._get_released_date()
        self.price = self._get_price()
        self.review = self._get_review()
    ...
    def _get_price(self):
        ...
        if price_container:
            price_text = BSoupHandler.get_text(soup=normal_price_container)
    ...
    def get_target_info(self):
        ...
        ImageHandler.download_image(
            target_id, self.is_bundle, self.token, self.path)
        target_info = {
            'id': target_id,
            'title': self.title,
